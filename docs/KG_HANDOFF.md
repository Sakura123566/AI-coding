...
> **v0.4 变更提醒**：`/api/kg/*` 现在**需要登录**（`Authorization: Bearer <token>`），
> `user_id` 不再由 URL 参数指定——但**响应里仍然照旧回传 `user_id`**，所以老客户端
> 读 `response.user_id` 不会失效。字段级契约以 `docs/contract/*.json` 为准，CI 会逐字段比对。
...
| 字段 | 含义 | 建议用法 |
|---|---|---|
| `id` | 后端的节点主键 | 要拉某个词的**详情 / 对话回溯 / 利弊分析**时用它调 `/api/kg/nodes/{id}`；**构图节点名仍建议用 `term`** |
