...
from src.ui.shared.emotion_mapping import DEFAULT_EMOTION, normalize_emotion_name
from src.ui.shared.bridge import EventBridge
from src.ui.shared.models import (
    ActivationModel,
    ConversationModel,
    MainModel,
    SettingsModel,
...
class ViewManager(QObject):
    ...
    def __init__(self, event_bus: EventBus):
        ...
        self._running = False
        self._current_avatar_state = DEFAULT_EMOTION
        self._idle_rotation_task: asyncio.Task | None = None
        self._idle_rotation_interval = 8.0
    ...
    def _subscribe_events(self):
        ...
        self._event_bus.on(Events.UI_CONVERSATION_MESSAGE, self._on_conversation_message)
    ...
    async def start(self, mode: str = "gui"):
        ...
        # 设置默认表情
        self._set_avatar_state(DEFAULT_EMOTION)
    ...
    def _inject_context(self):
        ...
        ctx.setContextProperty("conversationModel", self._conversation_model)
    ...
    def _unsubscribe_events(self) -> None:
        ...
        subscriptions = (
            ...
            (Events.UI_CONVERSATION_MESSAGE, self._on_conversation_message),
    ...
    def _set_avatar_state(self, emotion: str) -> None:
        """设置 Avatar 状态并维护 idle 轮换."""
        avatar_state = normalize_emotion_name(emotion)
        self._current_avatar_state = avatar_state
        url = self._emotion_service.get_emotion_url(avatar_state)
        self._main_model.set_emotion_url(url)

        if avatar_state == DEFAULT_EMOTION:
            self._start_idle_rotation()
        else:
            self._stop_idle_rotation()

    def _start_idle_rotation(self) -> None:
        if self._idle_rotation_task and not self._idle_rotation_task.done():
            return

        self._idle_rotation_task = self._tasks.spawn(
            self._rotate_idle_emotion(),
            name="ui:idle_emotion_rotation",
        )

    def _stop_idle_rotation(self) -> None:
        if self._idle_rotation_task and not self._idle_rotation_task.done():
            self._idle_rotation_task.cancel()
        self._idle_rotation_task = None

    async def _rotate_idle_emotion(self) -> None:
        """空闲状态下定期轮换 idle 资源."""
        try:
            while self._running and self._current_avatar_state == DEFAULT_EMOTION:
                await asyncio.sleep(self._idle_rotation_interval)
                if self._current_avatar_state != DEFAULT_EMOTION:
                    return
                url = self._emotion_service.get_emotion_url(DEFAULT_EMOTION)
                self._main_model.set_emotion_url(url)
